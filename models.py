    ...
    def __init__(
        ...
        for i in range(depth):
            self.down_path.append(
                UNetConvBlock_(prev_channels, 2 ** (wf + i), padding, batch_norm)
            )
        ...
        for i in reversed(range(depth - 1)):
            self.up_path.append(
                UNetUpBlock_(prev_channels, 2 ** (wf + i), up_mode, padding, batch_norm)
            )
            prev_channels = 2 ** (wf + i)
...
class UNetConvBlock_(nn.Module):
    def __init__(self, in_size, out_size, padding, batch_norm):
        super(UNetConvBlock_, self).__init__()
        block = []
...
class UNetUpBlock_(nn.Module):

    def __init__(self, in_size, out_size, up_mode, padding, batch_norm):
        super(UNetUpBlock_, self).__init__()
        if up_mode == 'upconv':
        ...
        self.conv_block = UNetConvBlock_(in_size, out_size, padding, batch_norm)
